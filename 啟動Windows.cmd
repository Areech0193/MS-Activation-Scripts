...
if not %errorlevel%==0 (
echo:
echo 錯誤：這不是一個正確的檔案。 它有 LF 行結束問題。
echo:
popd
...
cls
color 07
title  Microsoft Activation Scripts AIO (微軟啟用指令碼多合一)
...
set "nceline=echo: &echo ==== 錯誤 ==== &echo:"
set "eline=echo: &call :_color %Red% "==== 錯誤 ====" &echo:"
...
%nceline%
echo 檢測到不支援的操作系統版本。
echo 專案僅支援 Windows 7/8/8.1/10/11 及其對應的伺服器版本。
goto MASend
)

for %%# in (powershell.exe) do @if "%%~$PATH:#"=="" (
%nceline%
echo 系統中沒有安裝 Powershell。
echo 正在中止...
goto MASend
)
...
%nceline%
echo 指令碼從臨時資料夾啟動，
echo 很可能是直接從壓縮包中執行指令碼。
echo:
echo 提取壓縮包中的指令碼檔案到資料夾再啟動指令碼。
goto MASend
)
)
...
cls
color 07
title  Microsoft Activation Scripts AIO (微軟啟用指令碼多合一) 1.7
mode 76, 30
...
echo:             [2] KMS38       ^|  Windows           ^|至 2038 年
echo:             [3] 線上 KMS    ^|  Windows / Office  ^|    180 天
...
echo:       ______________________________________________________________
echo: 
echo:                      		Cvo Production(R)
echo: 
echo:       ______________________________________________________________
echo:
call :_color2 %_White% "          " %_Green% "輸入選單項 [1,2,3,4,5,6,0] :"
choice /C:1234560 /N
set _erl=%errorlevel%
...
echo:
echo:             [1] 啟用故障排除
echo:             [2] 更改 Windows 版本
echo:
echo:             [3] 提取 $OEM$ 資料夾
echo:             [4] 輸入 Windows HWID 金鑰
echo:             [5] 啟用狀態 [vbs]
echo:             __________________________________________________      
echo:                                                                     
echo:             [0] 返回主選單
echo:       ______________________________________________________________
echo:
call :_color2 %_White% "           " %_Green% "輸入選擇的選單項 [1,2,3,4,5,0] :"
choice /C:123450 /N
set _erl=%errorlevel%
...
echo:             [3] 線上 KMS
echo:             
echo:             [4] HWID  ^(Windows^) ^+ 線上 KMS ^(Office^)
echo:             [5] KMS38 ^(Windows^) ^+ 線上 KMS ^(Office^)
echo:             __________________________________________________      
echo:                                                                   
echo:             [0] 返回
echo:       ______________________________________________________________
echo:  
call :_color2 %_White% "           " %_Green% "輸入選擇的選單項:"
choice /C:123450 /N
set _erl=%errorlevel%

if %_erl%==6 goto:Extras
if %_erl%==5 (set "_oem=KMS38 [Windows] + 線上 KMS [Office]" & set "para=/KMS38 /KMS-ActAndRenewalTask /KMS-Office" &goto:Extract$OEM$3)
if %_erl%==4 (set "_oem=HWID [Windows] + 線上 KMS [Office]" & set "para=/HWID /KMS-ActAndRenewalTask /KMS-Office" &goto:Extract$OEM$3)
if %_erl%==3 (set "_oem=線上 KMS" & set "para=/KMS-ActAndRenewalTask /KMS-WindowsOffice" &goto:Extract$OEM$3)
...
call :_color %Green% "$OEM$ 資料夾成功在桌面建立。"
)
echo "%_oem%" | find /i "KMS38" 1>nul && (
echo:
echo 注意: 如果需要用 KMS38 方式啟用伺服器核心版，
echo       請從 MAS 單獨檔案版中提取。
)
...
set "nceline=echo: &echo ==== 錯誤 ==== &echo:"
set "eline=echo: &call :dk_color %Red% "==== 錯誤 ====" &echo:"
...
%eline%
echo 檢測到不支援的操作系統版本。
echo 專案支援 Windows 10/11。
goto dk_done
)

if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-Server*Edition~*.mum" (
%eline%
echo Windows Server 不支援 HWID 啟用。
echo 使用 KMS38 或 KMS 啟用。
goto dk_done
)
...
echo:
echo 錯誤程式碼: %errorlevel%
call :dk_color %Red% "啟動 [sppsvc] 服務失敗, 其餘過程可能需要很長時間..."
echo:
)
...
%eline%
echo [%winos% ^| %winbuild%]
echo 評估版無法啟用，下載並安裝完全版本的 Windows 操作系統。
echo:
...
echo 檢查操作系統資訊                 [%winos% ^| %winbuild% ^| %arch%]
...
set error=1
if defined _intcon (
call :dk_color %Red% "檢查網際網路連線                   [連線 licensing.mp.microsoft.com 失敗]"
call :dk_color %Magenta% "確保系統未連線受限網路 [Office/College] ， 網址沒有被阻止。"
) else (
call :dk_color %Red% "檢查網際網路連線                   [未連線]"
)
)
...
call :dk_color %Red% "正在檢查 HWID 啟用的備用版本     [%altedition% 啟用 ID 未找到]"
if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-*EvalEdition~*.mum" (
call :dk_color %Magenta% "發現評估版 Windows, 安裝完整版本的 Windows。 https://massgrave.dev/"
)
)
...
echo 在支援的產品列表中找不到此產品。
echo 確保您使用的是指令碼的最新版本。
echo https://massgrave.dev
echo:
goto dk_done
)
...
call :dk_color %Magenta% "[%altedition%] 版本產品金鑰將用於啟用 HWID 啟用。"
echo:
)
...
if %errorcode% EQU 0 (
call :dk_refresh
echo 安裝通用產品金鑰                 [%key%] [成功]
) else (
set error=1
call :dk_color %Red% "安裝通用產品金鑰          [%key%] [失敗] %errorcode%"
if defined applist if defined actidnotfound call :dk_color %Red% "找不到此金鑰的啟用ID。確保您使用的是最新版本的 MAS。"
)
...
if !errorlevel! EQU 0 (
echo 將 Windows 區域更改為美國        [成功]
) else (
call :dk_color %Red% "將 Windows 區域更改為美國          [失敗]"
)
)
...
call :dk_color %Red% "產生 GenuineTicket.xml           [失敗]"
echo [%encoded%]
if exist "%tdir%\Genuine*" del /f /q "%tdir%\Genuine*" %nul%
goto :dl_final
) else (
echo 產生 GenuineTicket.xml           [成功]
)
...
%_xmlexist% (
if exist "%tdir%\*.xml" del /f /q "%tdir%\*.xml" %nul%
call :dk_color %Red% "安裝 GenuineTicket.xml           [重啟 ClipSVC 服務失敗, 稍等...]"
)
)
...
call :dk_checkperm
if defined _perm (
call :dk_color %Green% "%winos% 已永久啟用。"
goto :dl_final
)
...
call :dk_color %Red% "刪除註冊表項                     [失敗] [!_ident!]"
) || (
echo 刪除註冊表項                     [成功] [!_ident!]
)
...
call :dk_color %Red% "啟用失敗 %error_code%"
if defined notworking (
call :dk_color %Magenta% "在編寫本指令碼時，此產品不支援 HWID 啟用。"
) else (
call :dk_color2 %Magenta% "檢視此頁面以獲取幫助" %_Yellow% " https://massgrave.dev/troubleshoot"
)
)
...
if !errorlevel! EQU 0 (
echo 還原 Windows 區域                [成功]
) else (
call :dk_color %Red% "還原 Windows 區域                [失敗] [%name%-%nation%]"
)
)

if %osSKU%==175 call :dk_color %Red% "ServerRdsh 版本在非 azure 平臺不正式支援啟用。"
...
if defined serv_csts echo 啟用禁用的服務              [成功] [%serv_csts%]

if defined serv_cste (
echo %serv_cste% | findstr /i "ClipSVC sppsvc" %nul% && (
call :dk_color %Red% "啟用禁用的服務              [失敗] [%serv_cste%] [重啟系統]"
) || (
call :dk_color %Red% "啟用禁用的服務              [失敗] [%serv_cste%]"
)
)
...
if defined serv_e (
set error=1
call :dk_color %Red% "啟動服務                       [失敗] [%serv_e%]"
)
...
if %errorlevel% NEQ 0 (
set error=1
call :dk_color %Red% "檢查 WMI                            [未響應] %_wmic%"
)


if not "%regSKU%"=="%wmiSKU%" (
set error=1
call :dk_color %Red% "檢查 WMI/REG SKU                    [發現差異 - WMI:%wmiSKU% Reg:%regSKU%]"
)
...
if %error_code% NEQ 0 (
call :dk_color %Red% "檢查 DISM                           [未響應] %error_code%"
)


if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-*EvalEdition~*.mum" (
call :dk_color %Red% "檢查 評估包                  [非評估許可證安裝在評估 Windows中]"
)
...
if %error_code% NEQ 0 (
set error=1
call :dk_color %Red% "檢查 slmgr /dlv                     [未響應] %error_code%"
)


reg query "HKU\S-1-5-20\Software\Microsoft\Windows NT\CurrentVersion\SoftwareProtectionPlatform\PersistedTSReArmed" %nul% && (
set error=1
call :dk_color %Red% "檢查 Rearm                          [需要重啟系統]"
)


reg query "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ClipSVC\Volatile\PersistedSystemState" %nul% && (
set error=1
call :dk_color %Red% "檢查 ClipSVC                        [需要重啟系統]"
)
...
call :dk_color %Red% "檢查 SkipRearm                      [未找到預設 0 值，正在更改為 0]"
...
if %token%==0 (
set error=1
call :dk_color %Red% "檢查 SPP tokens.dat                 [未發現]"
)

if not exist %SystemRoot%\system32\sppsvc.exe (
set error=1
call :dk_color %Red% "檢查 sppsvc.exe 檔案                [未發現]"
)

if /i %error_code% EQU 0xc0000022 (
echo "%serv_e%" | find /i "sppsvc" %nul% && (
call :dk_color %Magenta% "你可能使用了遊戲假離線，檢查 MAS 中的啟用故障排除選項。"
)
)
exit /b
...
call :dk_color %_Yellow% "按 任意鍵 %_exitmsg%..."
pause >nul
exit /b
...
%eline%
echo 檢測到不支援的操作系統版本。
echo 專案支援 Windows 10/11/Server, 14393 及更高版本。
goto dk_done
)
...
echo         ____________________________________________________________
echo: 
call :dk_color2 %_White% "              " %_Green% "輸入選擇的選單項 [1,2,0]"
choice /C:120 /N
set _el=!errorlevel!
if !_el!==3  exit /b
...
echo:
echo 錯誤程式碼: %errorlevel%
call :dk_color %Red% "無法啟動 [sppsvc] 服務，程序的其餘部分可能需要很長時間..."
echo:
)
...
echo 無法啟用伺服器評估版本，將其轉換為完整的伺服器操作系統。
echo:
echo 檢查附加選單中的 更改 Windows 版本 選項。
) else (
echo 無法啟用評估版本。下載並安裝 Windows 操作系統的完整版本。
echo:
echo https://massgrave.dev/
)
goto dk_done
...
%eline%
echo clipup.exe 在伺服器 Cor/Acor [非 GUI] 版本中不存在。
echo 這是 KMS38 啟用所必需的。
echo 請檢視下面的頁面，瞭解如何啟用。
...
echo:
for /f "skip=2 tokens=2*" %%a in ('reg query "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Environment" /v PROCESSOR_ARCHITECTURE') do set arch=%%b
echo 正在檢查操作系統資訊             [%winos% ^| %winbuild% ^| %arch%]
...
call :dk_color %Red% "檢查 KMS38 的備用版本   [%altedition% 啟用 ID 未發現]"
)
...
if !error_code! EQU 0 (
call :dk_refresh
echo 安裝 KMS 客戶端安裝金鑰          [%key%] [成功]
) else (
set error=1
call :dk_color %Red% "安裝 KMS 客戶端安裝金鑰          [%key%] [失敗] !error_code!"
)
)
...
if not defined app (
call :dk_color %Red% "檢查已安裝的 GVLK 啟用ID   [未發現] 正在中止..."
goto :dk_done
)
...
echo 新增特定 KMS 主機                [本地主機 127.0.0.2] [成功]
) else (
call :dk_color %Red% "新增特定 KMS 主機                [本地主機 127.0.0.2] [失敗]"
)
...
echo:
if exist "!_clipup!" (
echo 複製 clipup.exe 檔案到              [%systemroot%\System32\] [成功]
) else (
call :dk_color %Red% "複製 clipup.exe 檔案到              [%systemroot%\System32\] [失敗] Aborting..."
goto :k_final
)
)
...
echo 停止 sppsvc 服務                 [成功]
) || (
call :dk_color %Red% "停止 sppsvc 服務                 [失敗]"
)
...
%nul% reg delete "HKU\S-1-5-20\%specific_kms%" /f
%nul% reg query "HKLM\%specific_kms%" && (
call :dk_color %Red% "刪除新增的特定 KMS 主機    [失敗]"
) || (
...
%nul% reg delete "HKLM\%specific_kms%" /f
%nul% reg query "HKLM\%specific_kms%" && (
call :dk_color %Magenta% "使用 KMS 保護 KMS38                    [成功] [註冊表項已鎖定]"
) || (
call :dk_color %Red% "使用 KMS 保護 KMS38                    [鎖定註冊表項失敗]"
)
)
...
if defined a_cor (
if exist "%_clipup%" (
call :dk_color %Red% "刪除已複製的 clipup.exe 檔案         [失敗]"
) else (
echo 刪除已複製的 clipup.exe 檔案         [成功]
)
)

if %osSKU%==175 call :dk_color %Red% "ServerRdsh 版本不正式支援在非 azure 平臺上啟用。"
...
echo:
%nul% reg query "HKLM\%specific_kms%" && (
call :dk_color %Red% "刪除特定 KMS 主機              [失敗]"
) || (
echo 刪除特定 KMS 主機              [成功]
)
...
call :dk_color %Green% "%winos% 啟用至 !xpr!"
exit /b
...
echo 檢視此頁面以獲取幫助 https://massgrave.dev/troubleshoot
echo 正在中止...
goto Done
)
...
echo.              [3] 啟用 - 全部
echo.
if defined _tskinstalled call :_color2 %_White% "              [4] 安裝自動續期      " %_Green% "[已安裝]"
if defined _oldtsk       call :_color2 %_White% "              [4] 安裝自動續期      " %_Red% "[舊版 已安裝]"
if not defined _tskinstalled if not defined _oldtsk echo.              [4] 安裝自動續期
echo.              [5] 解除安裝
...
call :_color2 %_White% "              [6] 啟用除錯模式         " %_Red% "[Yes]"
)
if %vNextOverride% EQU 1 (
if %sub_next% EQU 1 (
call :_color2 %_White% "              [7] 覆蓋 Office vNext    " %_Red% "[Yes]"
) else (
echo               [7] 覆蓋 Office vNext    [Yes]
)
) else (
if %sub_next% EQU 1 (
call :_color2 %_White% "              [7] 覆蓋 Office vNext    " %_Yellow% "[No]"
) else (
echo               [7] 覆蓋 Office vNext    [No]
)
)
echo.              _______________________________________________       
echo.
...
echo.       ______________________________________________________________
echo.
call :_color2 %_White% "           " %_Green% "輸入選擇的選單項 [1,2,3,4,5,6,7,0]"
choice /C:12345670 /N
set _el=%errorlevel%
...
set "_mO21a=檢測到 Office 2021 C2R 零售版已啟用"
set "_mO19a=檢測到 Office 2019 C2R 零售版已啟用"
set "_mO16a=檢測到 Office 2016 C2R 零售版已啟用"
set "_mO15a=檢測到 Office 2013 C2R 零售版已啟用"
set "_mO21c=檢測到 Office 2021 C2R 零售版無法轉換為批量版"
set "_mO19c=檢測到 Office 2019 C2R 零售版無法轉換為批量版"
set "_mO16c=檢測到 Office 2016 C2R 零售版無法轉換為批量版"
set "_mO15c=檢測到 Office 2013 C2R 零售版無法轉換為批量版"
set "_mO14c=檢測到 Office 2010 C2R 零售版不被支援"
set "_mO14m=檢測到 Office 2010 MSI 零售版不被支援"
set "_mO15m=檢測到 Office 2013 MSI 零售版不被支援"
set "_mO16m=檢測到 Office 2016 MSI 零售版不被支援"
set "_mOuwp=檢測到 Office 365/2016 UWP版不被支援"
...
if %_Debug% EQU 0 (
  ...
  echo.
  echo 正在除錯模式下執行。。。
  if not defined _args (echo 完成後，視窗將關閉) else (echo 請稍候...)
  echo.
  echo 將除錯日誌寫入:
...
:Main
if defined EditionID (set "_winos=Windows %EditionID% edition") else (set "_winos=檢測到 Windows")
...
if %ActOffice% EQU 0 (echo.&echo Office 啟用已關閉...)
...
if %WinVL% EQU 0 (
if %ActWindows% EQU 0 (
  echo.&echo Windows 啟用已關閉...
  ) else (
  if %SSppHook% EQU 0 (
    echo.&echo %_winos% %nKMS%
    if defined _eval echo %nEval%
    ) else (
    echo.&echo 檢查 KMS 啟用 ID 失敗&echo 檢視此頁面以獲取幫助 https://massgrave.dev/troubleshoot &call :CheckWS
    exit /b
    )
  )
)
...
if %W1nd0ws% EQU 1 if %ActWindows% EQU 0 (echo.&echo Windows 啟用已關閉...)
...
if %winbuild% GEQ 9200 (
  if %OffUWP% EQU 0 (echo.&echo 未檢測到已安裝 Office 2013-2021...) else (echo.&echo %_mOuwp%)
  exit /b
  )
if %winbuild% LSS 9200 (if %loc_off14% EQU 0 (echo.&echo 未檢測到已安裝 Office %aword% ...&exit /b))
)
...
if %WinPerm% EQU 1 (
for /f "tokens=2 delims==" %%x in ('%_qr%') do echo 檢查： %%x
echo 產品已永久啟用。
exit /b
)
call :insKey
...
if %OsppHook% EQU 0 (echo.&echo 未檢測到已安裝 Office %aword% ...&exit /b)
if %winbuild% GEQ 9200 if %loc_off14% EQU 0 (echo.&echo 未檢測到已安裝 Office %aword%...&exit /b)
set err_offsvc=0
net start osppsvc /y %_Nul3% || (
sc start osppsvc %_Nul3%
if !errorlevel! EQU 1053 set err_offsvc=1
)
if %err_offsvc% EQU 1 (echo.&echo 錯誤: osppsvc 服務未執行...&exit /b)
...
set "_key="
set "_qr=%_zz7% %spp% %_zz2% %_zz5%ID='%app%'%_zz6% %_zz3% Name %_zz8%"
if %ESU_ADD% EQU 0 for /f "tokens=2 delims==" %%x in ('%_qr%') do echo 安裝 Key: %%x
if %ESU_ADD% EQU 1 for /f "tokens=2 delims==f" %%x in ('%_qr%') do echo 安裝 Key: %%x
set ESU_ADD=0
call :keys %app%
...
for /f "tokens=2 delims==" %%x in ('%_qr%') do echo 檢查: %%x
echo 產品已 KMS 2038 啟用。
set _keepkms38=1
exit /b
)
set "_qr=%_zz7% %spp% %_zz2% %_zz5%ID='%app%'%_zz6% %_zz3% Name %_zz8%"
if %act_attempt% LSS 1 (
if %ESU_ADD% EQU 0 for /f "tokens=2 delims==" %%x in ('%_qr%') do echo 啟用: %%x
if %ESU_ADD% EQU 1 for /f "tokens=2 delims==f" %%x in ('%_qr%') do echo 啟用: %%x
)
...
echo 產品啟用成功，但剩餘時間未能增加。
if %OSType% EQU Win7 echo 這可能與 KB4487266 中描述的錯誤有關
exit /b
)
set Act_OK=0
...
call :_color %_Green% "產品啟用成功"
echo 剩餘期限: %gpr2% 天 ^(%gpr% 分^)
set /a act_attempt=0
exit /b
)
...
cmd /c exit /b %ERRORCODE%
if %ERRORCODE% NEQ 0 (
call :_color %_Red% "產品啟用失敗: 0x!=ExitCode!"
) else (
call :_color %_Red% "產品啟用失敗"
)
echo 剩餘期限: %gpr2% 天 ^(%gpr% 分^)
set S_OK=0
set act_failed=1
...
if "%_ProductIds%"=="" (
if %_Office15% EQU 0 (echo 錯誤: Office C2R 產品ID 未檢測到&goto :%_fC2R%) else (goto :Reg15istry)
)
if not exist "%_LicensesPath%\ProPlus*.xrm-ms" (
if %_Office15% EQU 0 (echo 錯誤: Office C2R 許可檔案未檢測到&goto :%_fC2R%) else (goto :Reg15istry)
)
if not exist "%_Integrator%" (
if %_Office15% EQU 0 (echo 錯誤: Office C2R 許可整合器未檢測到&goto :%_fC2R%) else (goto :Reg15istry)
)
...
if "%_Product15Ids%"=="" (
if %_Office16% EQU 0 (echo 錯誤: Office 2013 C2R 產品ID未檢測到&goto :%_fC2R%) else (goto :CheckC2R)
)
if not exist "%_Licenses15Path%\ProPlus*.xrm-ms" (
if %_Office16% EQU 0 (echo 錯誤: Office 2013 C2R 許可檔案未檢測到&goto :%_fC2R%) else (goto :CheckC2R)
)
if %winbuild% LSS 9200 if not exist "%_OSPP15VBS%" (
if %_Office16% EQU 0 (echo 錯誤: Office 2013 C2R 許可工具 OSPP.vbs 未檢測到&goto :%_fC2R%) else (goto :CheckC2R)
)
...
echo.
echo Microsoft Office 使用 vNext 許可證啟用。
)
if %sub_proj% EQU 1 (
  for %%a in (%_PrjSKU%) do set _%%a=0
echo.
echo Microsoft Project 使用 vNext 許可證啟用。
)
if %sub_vsio% EQU 1 (
  for %%a in (%_VisSKU%) do set _%%a=0
echo.
echo Microsoft Visio 使用 vNext 許可證啟用。
)
...
echo 將不存在的IP地址 10.0.0.10 保留為 KMS 伺服器。
) else (
call :Clear-KMS-Cache
)
)

if defined _tskinstalled echo 找到續期任務，將線上 KMS IP 保留在系統中。
if defined _oldtsk echo 找到續期任務，將線上 KMS IP 保留在系統中。
...
echo 發現網際網路，但無法連線到埠 1688 上的KMS伺服器。
echo.
echo 確保未連線到受限制的網際網路[Office/College]，
echo 或者埠 1688 未防火墻被阻止。
echo.
echo 使用其他網際網路連線或使用離線 KMS 啟用
echo https://github.com/abbodi1406/KMS_VL_ALL_AIO
exit /b
)

if [%ERRORCODE%]==[-1073418124] (
echo KMS 伺服器埠 1688 測試通過。
echo 確保系統檔案未被防火墻阻止。
echo.
echo 如果問題仍然存在，請嘗試離線 KMS 啟用器，
echo https://github.com/abbodi1406/KMS_VL_ALL_AIO
echo.
)

echo KMS server is not an issue in this case.
call :_color2 %Magenta% "檢視此頁面以獲取幫助" %_Yellow% " https://massgrave.dev/troubleshoot"
exit /b
...
set error_=9
echo 完全清除KMS快取失敗。
reg query "HKLM\%SPPk%\%_wApp%" /s 2>nul | findstr /i "127.0.0.2" >nul && echo KMS38 啟用被鎖定。
) || (
echo 已成功清除 KMS 快取。
)
exit /b
...
echo ## 注意 ##
echo.
echo ?了確保 Office 程式不會顯示非正版提示，,
echo 請執行啟用選項一次，以後不要解除安裝。
...
echo __________________________________________________________________________________________
echo.
call :_color %Green% "線上 KMS 已成功完全解除安裝。"
echo __________________________________________________________________________________________
)
...
if defined error_ (
%eline%
echo 無法完全清除 KMS 相關資料夾/任務。
echo 執行「解除安裝」選項，然後重試。
goto :RenDone
)
...
echo %_dest%\Info.txt
echo.
(if defined ActTask (echo Scheduled Tasks created:) else (echo 建立計劃任務:))
echo \啟用-續期 [每週]
if defined ActTask (echo \啟用-僅一次)
echo __________________________________________________________________________________________
echo.
echo 說明:
echo 如果發現網際網路連線，啟用將每週更新。
echo ________________________________________________________________________________________
echo.
if defined ActTask (
call :_color %Green% "已成功建立續期和啟用任務。"
) else (
call :_color %Green% "已成功建立續期任務。"
)
echo.
call :_color %Gray% "確保從上一級選單至少執行一次啟用。"
echo __________________個_______________________________________________________________________
)
...
(
echo   此指令碼的用途是使用聯機 KMS 啟用/續期 您的 Windows/Office 許可證。
echo:
echo   如果已建立 更新/啟用 計劃任務，下列將存在，
echo:
echo   - 計劃任務
echo     啟用續期    [續期 / 每週]
echo     啟用-僅一次   [啟用任務 - 啟用后自我刪除]
echo     計劃任務僅在系統連線到網際網路時執行。
echo:
echo   - 檔案
...
echo:
echo   線上 KMS 啟用指令碼是 [MAS] 專案的一部分。
echo:   
echo   主頁: massgrave.dev
echo   郵箱: windowsaddict@protonmail.com
)>"%_dest%\Info.txt"
exit /b
...
<Task version="1.3" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <RegistrationInfo>
    ...
    <Version>1.0</Version>
    <Description>線上 KMS 啟用續期-每週任務</Description>
...
%nul% %psc% "$f=[io.file]::ReadAllText('!_batp!',[Text.Encoding]::Default) -split \":%~1\:.*`r`n\"; [io.file]::WriteAllText('%~2',$f[1].Trim(),[Text.Encoding]::Default);"
exit /b
...
if %_wmic% EQU 1 for /f "tokens=2 delims==" %%x in ('"wmic path !_path! where ID='!_actid!' get Name /VALUE" 2^>nul') do call echo 啟用: %%x
if %_wmic% EQU 0 for /f "tokens=2 delims==" %%x in ('%psc% "(([WMISEARCHER]'SELECT Name FROM !_path! WHERE ID=''!_actid!''').Get()).Name | %% {echo ('Name='+$_)}" 2^>nul') do call echo 啟用: %%x
exit /b
...
echo 產品啟用成功，但剩餘時間未能增加。
if [%1]==[act_win] if %winbuild% LSS 9200 echo 這可能與 KB4487266 中描述的錯誤有關
set _tserror=1
exit /b
)
...
echo %line2%
if %_sO15vbs% EQU 0 (
echo ***              Office 2016 %_bit%-bit 狀態               ***
) else (
echo ***               Office 2013/2016 狀態                ***
)
echo %line2%
...
echo %line2%
if %_sO15vbs% EQU 0 (
echo ***              Office 2016-2021 C2R 狀態             ***
) else (
echo ***                Office 2013-2021 狀態               ***
)
echo %line2%
cscript //nologo "!office!\ospp.vbs" /dstatus
)
...
echo %Description%| findstr /i TIMEBASED_ 1>nul && (set cTblClient=1&set _mTag=基於時間)
echo %Description%| findstr /i VIRTUAL_MACHINE_ACTIVATION 1>nul && (set cAvmClient=1&set _mTag=自動虛擬機器)
...
if %GracePeriodRemaining% EQU 0 (
  if %winID% EQU 1 (set "ExpireMsg=此計算機已永久啟用。") else (set "ExpireMsg=產品已永久啟用。")
  ) else (
  set "LicenseMsg=%_mTag% 啟用期限: %GracePeriodRemaining% 分鐘 (%_gpr% 天)"
  if defined _xpr set "ExpireMsg=%_mTag% 啟用期限 %_xpr%"
  )
)
if %LicenseStatus% EQU 2 (
set "License=初始寬限期"
if defined _xpr set "ExpireMsg=初始寬限期結束 %_xpr%"
)
if %LicenseStatus% EQU 3 (
set "License=額外寬限期 (KMS許可證過期或硬體超出容差)"
if defined _xpr set "ExpireMsg=額外寬限期結束 %_xpr%"
)
if %LicenseStatus% EQU 4 (
set "License=非真實寬限期。"
if defined _xpr set "ExpireMsg=非真實寬限期結束 %_xpr%"
)
if %LicenseStatus% EQU 6 (
set "License=延長寬限期"
if defined _xpr set "ExpireMsg=延長的寬限期結束 %_xpr%"
)
if %LicenseStatus% EQU 5 (
set "License=通知"
  if "%LicenseReason%"=="C004F200" (set "LicenseMsg=通知原因: 0xC004F200 (非正品)."
  ) else if "%LicenseReason%"=="C004F009" (set "LicenseMsg=通知原因: 0xC004F009 (寬限期已過期)."
...
for /f "tokens=* delims=" %%# in ('%_qr%') do set "%%#"
if /i %KeyManagementServiceHostCaching%==True (set KeyManagementServiceHostCaching=啟用) else (set KeyManagementServiceHostCaching=禁用)
...
echo 描述: %Description%
echo 啟用 ID: %ID%
echo 擴充套件 PID: %ProductKeyID%
if defined ProductKeyChannel echo 產品金鑰渠道: %ProductKeyChannel%
echo 部分產品金鑰: %PartialProductKey%
echo 許可狀態: %License%
if defined LicenseMsg echo %LicenseMsg%
if not %LicenseStatus%==0 if not %EvaluationEndDate:~0,8%==16010101 echo 評估結束日期: %EvaluationEndDate:~0,4%-%EvaluationEndDate:~4,2%-%EvaluationEndDate:~6,2% %EvaluationEndDate:~8,2%:%EvaluationEndDate:~10,2% UTC
if not defined cKmsClient (
if defined ExpireMsg echo.&echo.    %ExpireMsg%
exit /b
)
if defined VLActivationTypeEnabled echo 已配置啟用型別: %VLActivationType%
echo.
if not %LicenseStatus%==1 (
echo 啟用產品以更新 KMS 客戶端資訊值.
exit /b
)
echo 最新啟用資訊:
echo 金鑰管理服務客戶端資訊
...
echo.    KMS 計算機擴充套件 PID: %KeyManagementServiceProductKeyID%
echo.    啟用間隔: %VLActivationInterval% 分鐘
echo.    續期間隔: %VLRenewalInterval% 分鐘
echo.    KMS 主機快取: %KeyManagementServiceHostCaching%
if defined KeyManagementServiceLookupDomain echo.    KMS SRV 記錄查詢域: %KeyManagementServiceLookupDomain%
if defined ExpireMsg echo.&echo.    %ExpireMsg%
exit /b
...
%eline%
echo Desktop location was not detected, 正在中止...
goto at_done
)
...
echo:                                                   
call :_color2 %_White% "             [1] " %_Green% "幫助"
...
echo:
echo:             [4] 重建 許可令牌
echo:             [5] 重建 ClipSVC 許可證
echo:             [6] 清除 Office vNext 許可證
echo:             ___________________________________________________
echo:                                                                      
echo:             [7] 重建 WMI 儲存庫
echo:             [8] 修復: 遊戲假離線
echo:             [9] 修復: Windows 7 中的 KB971033 引起的問題
echo:             [G] 修復：Office 非正版
echo:             [E] 導出事件檢視器日誌
echo:             ___________________________________________________
echo:
echo:             [0] %_exitmsg%
...
call :_color2 %_White% "     " %Red% "檢查網際網路連線  [未連線]"
)

echo %line%
echo:
echo      Dism 使用 Windows Update 提供修復損壞所需的檔案。
echo      這需要5-15分鐘或更長時間...
echo %line%
echo:
echo      說明:
echo:
call :_color2 %_White% "     - " %Gray% "確保網際網路已連線。"
call :_color2 %_White% "     - " %Gray% "確保Windows更新正常工作。"
echo:
echo %line%
echo:
choice /C:09 /N /M ">    [9] 繼續 [0] 返回 : "
...
echo %line%
echo:    
echo      系統檔案檢查器將修復丟失或損壞的系統檔案。
echo      這將需要10-15分鐘或更長時間...
echo:
echo      如果SFC無法修復某個問題，請再次執行該命令，看看是否可以在下一次執行。
echo      有時，可能需要執行 sfc/scannow 命令三次，然後每次重新啟動電腦，
echo      才能完全修復所有可能的問題。
echo:   
echo %line%
echo:
...
echo:
call :_color %Gray% "CBS 和主提取日誌被複制到桌面上的 AT_logs 資料夾。"
goto :at_back
...
cls
mode 98, 30
title  清除 Office vNext 許可證

echo:
echo %line%
echo:    
echo      此選項將清除 Office vNext (訂閱) 許可證
echo:
echo      出現以下情況時需要使用此選項：
echo          - 由於現有訂閱許可證，KMS 選項未啟用 Office
echo          - KMS 選項已啟用Office，但 Office 啟用頁面未顯示已啟用
echo:   
echo %line%
echo:
choice /C:09 /N /M ">    [9] 繼續 [0] 返回 : "
...
echo %line%
echo:
call :_color %Magenta% "清除 Office vNext 許可證"
echo:
...
echo 刪除失敗 - !ProgramData!\Microsoft\Office\Licenses\
) else (
echo 刪除資料夾 - !ProgramData!\Microsoft\Office\Licenses\
)

if exist "!_Local!\Microsoft\Office\Licenses\" (
echo 刪除失敗 - !_Local!\Microsoft\Office\Licenses\
) else (
echo 刪除資料夾 - !_Local!\Microsoft\Office\Licenses\
)
...
echo       - 它有助於排除啟用問題。
echo:
call :_color2 %_White% "      - " %Magenta% "此選項將,"
call :_color2 %_White% "        " %Magenta% "- 重置 Windows 和 Office，您需要重新啟用"
call :_color2 %_White% "        " %Magenta% "- 解除安裝 Office 許可證和金鑰"
call :_color2 %_White% "        " %Magenta% "- 清理 SPP-OSPP data.dat, tokens.dat, cache.dat"
call :_color2 %_White% "        " %Magenta% "- 觸發 Office 的修復選項"
echo:
call :_color2 %_White% "      - " %Red% "僅在必要時使用。"
echo:
echo %line%
echo:
choice /C:09 /N /M ">    [9] 繼續 [0] 返回 : "
...
echo %line%
echo:
call :_color %Magenta% "正在重建 SPP 許可令牌"
echo:

call :scandat check

if not defined token (
call :_color %Red% "未找到 tokens.dat 檔案。"
) else (
...
if defined token (
echo:
call :_color %Red% "刪除 .dat 檔案失敗。"
echo:
)

echo:
echo 重灌系統許可 [slmgr /rilc]...
cscript //nologo %windir%\system32\slmgr.vbs /rilc %nul%
if %errorlevel% NEQ 0 cscript //nologo %windir%\system32\slmgr.vbs /rilc %nul%
if %errorlevel% EQU 0 (
echo [成功]
...
echo:
if not defined token (
call :_color %Red% "重建 tokens.dat 檔案失敗。"
) else (
call :_color %Green% "tokens.dat 檔案重建成功。"
)
...
sc qc osppsvc %nul% || (
echo:
call :_color %Magenta% "未安裝基於 OSPP 的 Office"
call :_color %Magenta% "跳過重建 OSPP 令牌"
goto :repairoffice
)

call :_color %Magenta% "重建 OSPP 許可令牌"
echo:
...
echo:
echo 執行 osppsvc 服務產生 tokens.dat
...
echo %line%
echo:
call :_color %Magenta% "修復 Office 許可證"
echo:
...
if /i "%arch%"=="ARM64" (
echo:
echo 發現 ARM64 Windows 。
echo 你需要在 Windows 設定中為 Office 使用修復選項。
echo:
...
echo:
call :_color %_Yellow% "在彈出一個視窗中，選擇修復選項..."
call :_color %_Yellow% "按 任意鍵 繼續..."
echo:
pause >nul
)
...
if defined uwp16 (
echo:
echo 注意：正在跳過 Office 16.0 UWP 的修復。
echo       您需要在 Windows 設定中使用重置選項。
echo ________________________________________________________________
echo:
start ms-settings:appsfeatures
)
...
if defined c2r14 (
echo:
echo 注意：正在跳過 Office 14.0 C2R 的修復
echo       您需要使用 Windows 設定中的修復選項。
echo ________________________________________________________________
echo:
start appwiz.cpl
)
...
cls
mode 98, 30
title  重建 ClipSVC 許可證

if %winbuild% LSS 10240 (
%eline%
echo 檢測到不支援的操作系統版本。
echo 僅 Windows 10/11及其同級伺服器版本支援此命令...
goto :at_back
)
...
echo      說明:
echo:
echo       - 重建 ClipSVC 許可證有助於解決 HWID-KMS38 啟用問題。
echo:
echo       - 除非在 HWID-KMS38 啟用中遇到問題，否則不要執行此選項。
echo:
echo       - 建議在應用後重啟系統。
echo:
echo %line%
...
echo:
echo 正在刪除不穩定、受保護的註冊表項...
echo [%RegKey%]
reg query "%RegKey%" %nul% && (
call :_color %Red% "[失敗]"
echo 重啟系統，將自動刪除此註冊表項。
) || (
call :_color %Green% "[成功]"
)
...
echo:
echo 正在重啟 [ClipSVC wlidsvc LicenseManager sppsvc] 服務...
...
echo      說明:
echo:
echo       - 遊戲解鎖/假離線/清理通常會導致Windows啟用問題。
echo:
call :_color2 %_White% "      - " %Red% "僅當您使用過這些東西時才應用此修復程式。"
echo:
echo       - 此選項將修復檔案和註冊表許可權以及重建許可令牌.
echo:
echo       - 建議在應用後重啟系統。
echo:
echo %line%
...
%eline%
echo 檢測到不支援的操作系統版本。
echo 此選項僅在Windows 7 及同內核的伺服器上受支援
goto :at_back
)
...
echo      說明:
echo:
echo       - 此選項修復了 Windows 7 中更新 KB971033 導致的問題。
echo         https://support.microsoft.com/en-us/help/4487266
echo:
echo %line%
echo:
choice /C:01 /N /M ">    [1] 繼續 [0] 返回 : "
if %errorlevel%==1 goto at_menu

cls
echo:

echo 正在檢查更新 KB971033...
...
if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-Server*Edition~*.mum" (
%eline%
echo 不建議在 Windows Server 上重建 WMI， 正在中止...
goto :at_back
)
...
echo:
if defined error (
echo      WMI 狀態 - [未響應] %_wmic%
) else (
call :_color %_Green% "     WMI 狀態 - [工作中]"
)
echo:
echo      說明:
echo:
call :_color2 %_White% "      - " %Magenta% "重建 WMI 可能導致某些第三方應用在重新安裝之前無法工作。"
echo:       
call :_color2 %_White% "      - " %Red% "僅當 WMI 不工作時應用此修復程式。"
echo:
echo %line%
echo:
choice /C:09 /N /M ">    [9] 繼續 [0] 返回 : "
...
%eline%
echo 未安裝 Winmgmt 服務。 正在中止...
goto :at_back
)

echo 正在禁用 Winmgmt 服務...
sc config Winmgmt start= disabled %nul%
if %errorlevel% EQU 0 (
call :_color %Green% "[成功]"
) else (
call :_color %Red% "[失敗] 正在中止..."
goto :wmifixend
)

echo:
echo 正在停止 Winmgmt 服務...
...
call :_color %Green% "[成功]"
echo EventLogs_%_time%.cab 建立在桌面上的 AT_Logs 資料夾中。
) else (
call :_color %Red% "[失敗]"
)
...
%eline%
echo [%winos% ^| %winbuild% ^| SKU:%osSKU%]
echo 在 HWID 支援的產品列表中找不到此產品。
echo 確保你正在使用的指令碼是更新過的版本。
echo https://massgrave.dev
goto ins_done
)
...
%line%
echo:
echo 安裝 [%winos% ^| SKU:%osSKU% ^| %winbuild%] 金鑰
echo [%key%]
%line%
echo:
if not "%regSKU%"=="%wmiSKU%" (
echo 說明: SKU 值存在差異 - WMI:%wmiSKU% Reg:%regSKU%
echo       重啟系統以解決此問題
echo:
)
call :dk_color %_Green% "按 [1] 繼續 或 按 [0] %_exitmsg%"
choice /C:01 /N
...
if defined actidnotfound call :dk_color %Red% "找不到此金鑰的啟用ID。"
)
%line%
...
%nceline%
echo 未找到已更新的 Powershell。
echo:
echo 從下面網頁鏈接下載並安裝 Windows Management Framework 5.1。
...
echo DISM 命令失敗 [錯誤程式碼 - 0x!=ExitCode!]
echo 未正確檢測到操作系統版本。 正在中止...
goto ced_done
)
...
echo 目前版本 [%osedition% ^| %winbuild%] 不能更改為任何其他版本。
%line%
goto ced_done
)
...
%line%
echo:
call :dk_color %Gray% "您可以將目前版本 [%osedition%] 更改為以下版本之一。"
%line%
echo:
...
echo:
if defined note (
echo 註：CBS 升級方法適用於紫色版本。
echo:
)
call :dk_color %_Green% "輸入選項編號，然後按"回車":"
set /p inpt=
...
echo 無法從 pkeyhelper.dll 獲取產品金鑰 
echo 確保你正在使用的指令碼是最新的版本
echo https://massgrave.dev
goto ced_done
)
...
%eline%
echo 由於缺少非OEM金鑰，[%osedition%] 無法更改為 [%targetedition%] 版本。
echo 非 OEM 金鑰需要從核心版本更改為非核心版本。
goto ced_done
)
)
...
if not [%%#]==[] (
%eline%
echo 需要斷開網際網路連線才能將版本 [%osedition%] 更改為 [%targetedition%]
echo 斷開網際網路連線，然後按任意鍵...
pause >nul
goto ced_loop
)
)
...
echo:
echo 更改當前版本 [%osedition%] 為 [%targetedition%]
echo:

if %_changepk%==1 (
call :dk_color %_Green% "如果升級 Windows 中出現錯誤，您可以安全地忽略。"
call :dk_color %_Yellow% "但在這種情況下，您必須手動重啟系統。"
echo:
%psc% "$BLinfo = Get-BitLockerVolume -MountPoint "C:";$blinfo.ProtectionStatus" | find /i "On" 1>nul && (
call :dk_color %Red% "系統中的 Bitlocker 或裝置 加密已打開。"
echo:
echo 要麼使用其他CBS升級方法進行版本更改
echo 或者     確保您有恢復金鑰，您可能需要它
echo 或者     關閉 Bitlocker / 裝置 加密
echo:
)
call :dk_color %Magenta% "Important - Save your work before continue, system will auto reboot."
echo:
choice /C:21 /N /M "[1] 繼續 [2] 返回 : "
if !errorlevel!==1 exit /b
)
...
call :dk_color %Green% "[成功]"
echo:
call :dk_color %Gray% "需要重啟才能正確更改版本。"
) else (
...
echo:
echo 將目前版本 [%osedition%] 更改為 [%targetedition%]
echo:
call :dk_color %Magenta% "重要資訊-在繼續之前儲存您的工作，系統將自動重啟。"
if %winbuild% GEQ 17034 if %targetedition%==Professional echo           - 將安裝企業版金鑰，你可以稍後快速更改為專業版。
echo:
